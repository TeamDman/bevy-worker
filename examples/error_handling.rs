...
use bevy::prelude::*;
use bevy_worker::WorkerConfig;
use bevy_worker::WorkerPlugin;
...
fn handle_threadbound_message(
    ...
    if state.count == 13 {
        return Err(BevyError::from("Count reached 13!"));
    }
    reply_tx.send(GameboundMessage::Latest(state.count))?;
    Ok(())
}

/// For demonstration purposes, this only crashes on the 4th error
